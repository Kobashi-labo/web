...
function pickPersonNameJaFirst(p) {
  if (p == null) return "";
  if (typeof p === "object") {
    return (
      p.ja ||
...
function formatPresentationAuthors(item, { preferJa = false } = {}) {
  const people = getPresentationPeopleArray(item)
    .map((p) => (preferJa ? pickPersonNameJaFirst(p) : pickPersonName(p)))
    .filter(Boolean);
...
function toNumberedPresentationList(items, { preferJa = false } = {}) {
  const sorted = [...items].sort((a, b) => {
  ...
  const numbered = sorted.map((item, idx) => ({
    ...
    authors: formatPresentationAuthors(item, { preferJa }),
...
async function main() {
  ...
  const presInternational = toNumberedPresentationList(oralInternationalRaw, { preferJa: false });
  const presDomestic = toNumberedPresentationList(oralDomesticRaw, { preferJa: true });
  const invited = toNumberedPresentationList(invitedRaw, { preferJa: false });
